...
{
    if (status->deviceSpeed == 0x01) GUIEventMonitor::KernelEventMonitor->Log("High speed device attached!");
    if (status->deviceSpeed == 0x02) GUIEventMonitor::KernelEventMonitor->Log("Full speed device attached!");
    ...
    if (status->deviceSpeed == 0x1 || status->deviceSpeed == 0x2)
    {
        ...
        {
            GUIEventMonitor::KernelEventMonitor->Log("USB device was changed during processing.");
            return;
        }
        ...
        if (stat != 0) 
        {
            GUIEventMonitor::KernelEventMonitor->Log("USB Device Error!");
            return;
        }
        /////////////////////////////////////
        ...
        if (usbDescriptor == 0)
        { 
            GUIEventMonitor::KernelEventMonitor->Log("USB Memory error.");
            return;
        }
        ...
        if (usbConfig == 0)
        {
            ...
            for (MemoryChunk* chunk = (MemoryChunk*)(10*1024*1024); chunk->next != 0; chunk = chunk->next)
            {
                ...
                GUIEventMonitor::KernelEventMonitor->Log(bhh);
        ...
        {
            GUIEventMonitor::KernelEventMonitor->Log("No support for USB devices that have more than one interface.");
            MemoryManager::ActiveMemoryManager->free(usbDescriptor);
            return;
        }
        ...
        for (int index = 0; index < usbConfig->totalLength; )
        {
            if (otherDeviceData[index+1] == 0x04)  //Interface descriptor
            {
                ...
                if (id->Class == 0x08 && id->SubClass == 0x06 && id->Protocol == 0x50)
                {
                    ...
                    {
                        GUIEventMonitor::KernelEventMonitor->Log("This device does not contain valid data endpoints");
                        MemoryManager::ActiveMemoryManager->free(usbDescriptor);
                    ...
                    {
                        if (massStorage == 0) GUIEventMonitor::KernelEventMonitor->Log("USB Memory error 3.");
                        else GUIEventMonitor::KernelEventMonitor->Log("USB device removed during processing .");
                        MemoryManager::ActiveMemoryManager->free(usbDescriptor);
                        MemoryManager::ActiveMemoryManager->free(epin);
                        MemoryManager::ActiveMemoryManager->free(epout);
                        deviceAddresses[deviceAddress] = false;
                    ...
                    {
                        GUIEventMonitor::KernelEventMonitor->Log("USB Mass Storage Init error.");
                        MemoryManager::ActiveMemoryManager->free(usbDescriptor);
                        MemoryManager::ActiveMemoryManager->free(epin);
                        MemoryManager::ActiveMemoryManager->free(epout);
                ...
                {
                    GUIEventMonitor::KernelEventMonitor->Log("Unknown USB Interface.");                    
                }
            }
    ...
    {
        GUIEventMonitor::KernelEventMonitor->Log("USB Device Removed.");
    }
}
