...
int USBMassStorage::Initialize()
{
    ...
    while (((uint8_t*)statusBuffer)[12] != 0 && retries < 5)  //There was am error
    {
        ...
        if (status != 0)
        {
            GUIEventMonitor::KernelEventMonitor->Log("Mass storage Inquiry Error!");
            return status;
        }
    ...
    while (((uint8_t*)statusBuffer)[12] != 0 && retries < 5)  //There was am error
    {
        ...
        if (status != 0)
        {
            GUIEventMonitor::KernelEventMonitor->Log("Mass storage Test Unit Ready Error!");
            return status;
        }
        ...
        {       
            ...
            if (status != 0)
            {
                GUIEventMonitor::KernelEventMonitor->Log("Mass storage Request sense Error!");
                return status;
            }
        }
    ...
    while (((uint8_t*)statusBuffer)[12] != 0 && retries < 5)  //There was am error
    {
        ...
        if (status != 0)
        {
            GUIEventMonitor::KernelEventMonitor->Log("Mass storage Read Capacity Error!");
            return status;
        }
    ...
    status = Read(0, 1, (uint8_t*)dataBuffer);
    if (status != 0)
    {
        GUIEventMonitor::KernelEventMonitor->Log("Drive Read error!  : "); 
        // for (int i = 0; i < 13; i++) printHex8(((uint8_t*)statusBuffer)[i]);
        return status;
    }

    if (!USBMassStorageDevices::Devices->AddDevice(this)) return 0xff;
    GUIEventMonitor::KernelEventMonitor->Log("New USB Mass Storage Initialized.");
