...
: InterruptHandler(man, 0x20 + dev->interrupt_line)
{
    ...
    if (ext_cap_offset > 0)
    {
        ...
        if (i == 10) 
        {
            GUIEventMonitor::KernelEventMonitor->Log("USB Init error!");   
            return;
        }
    }
...
void EHCIDriver::Activate()
{
    ...
    GUIEventMonitor::KernelEventMonitor->Log("EHCI Driver activated!");
}
...
uint32_t EHCIDriver::HandleInterrupt(uint32_t esp)
{
	...
	if (int_val & 0x2)
	{
		GUIEventMonitor::KernelEventMonitor->Log("***USB ERROR***");
...
void EHCIDriver::CheckPort(int portno)
{
    ...
    if (OSEHCIDriver->DevicePresent(portno))
    {
        ...
        {
            GUIEventMonitor::KernelEventMonitor->Log("Low speed device attached!");
